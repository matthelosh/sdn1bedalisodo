...
const addAccount = async (item) => {
    loading.value = true;
    await axios
        .post(route("dashboard.guru.account.add", { _query: { id: item.id } }))
        .then((res) => {
            loading.value = false;
            router.reload({ only: ["gurus"] });
        })
...
const removeAccount = async (item) => {
    ...
    await axios
        .delete(
            route("dashboard.guru.account.remove", { _query: { id: item.id } })
        )
        .then((res) => {
            loading.value = false;
            router.reload({ only: ["gurus"] });
...
const inactiveGuru = async (item) => {
    loading.value = true;
    await router.put(route("dashboard.guru.status.toggle"), item, {
...
<template>
    ...
    <AdminLayout title="Data Guru">
        <div class="wrapper w-full oveflow-x-hidden p-3">
            <div
                class="toolbar w-full h-10 flex items-center justify-between p-3 bg-slate-600 text-white"
            >
                ...
                <button
                    class="py-1 px-3 rounded bg-slate-100 text-slate-800"
                    @click="showForm = true"
                >
                    Tambah
                </button>
            ...
            <div class="table w-full p-3 bg-white">
                <table class="table-border border-collapse w-full text-xs">
                    ...
                    <tbody>
                        <tr
                            v-for="(guru, g) in $page.props.gurus"
                            :key="g"
                        ...
                        >
                            ...
                            <td class="py-1 px-2 border text-center">
                                <button
                                    v-if="!guru.user"
                                    @click="addAccount(guru)"
                                >
                                    <Icon
                                        icon="mdi:account-plus"
                                        class="text-xl text-sky-400 hover:text-sky-600"
                                    />
                                </button>
                                <button
                                    v-if="guru.user"
                                    @click="removeAccount(guru)"
                                >
                                    <Icon
                                        icon="mdi:account-minus"
                                        class="text-xl text-red-400 hover:text-red-600"
                                    />
                                </button>
                                <button @click="inactiveGuru(guru)">
                                    <Icon
