...
const AturPeserta = defineAsyncComponent(() =>
    import("@/Components/Dashboard/Ekskul/AturPeserta.vue")
);
...
const getGurus = async () => {
    await axios
        .post(route("dashboard.guru.index"))
        .then((res) => {
...
const list = async () => {
    loading.value = true;
    await axios
        .post(route("dashboard.ekskul.index"))
        .then((res) => {
            loading.value = false;
...
const simpan = async () => {
    loading.value = true;
    await axios
        .post(route("dashboard.ekskul.store"), {
            ekskul: JSON.stringify(ekskul.value),
        })
        .then((res) => {
            loading.value = false;
...
const yakinHapus = async (item) => {
    if (
        await confirmDialog.value.open(
            `Yakin Menghapus Data Ekskul ${item.label}`
        )
    ) {
        hapus(item);
    } else {
...
const hapus = async (item) => {
    loading.value = true;
    await axios
        .delete(route("dashboard.ekskul.destroy", { id: item.id }))
        .then((res) => {
            list();
            loading.value = false;
        })
...
<template>
    ...
    <AdminLayout :title="pageTitle">
        <div
            class="toolbar sticky z-50 top-0 h-10 flex items-center justify-between p-3 bg-white"
        >
        ...
        <div class="content py-3 px-4">
            <AturPeserta
                ...
                v-if="selectedEkskul !== null"
            />
            <div
                class="table w-full overflow-auto bg-white shadow hover:shadow-lg"
                v-else
            >
                <table class="w-full border">
                    <thead>
                        <tr>
                            <th class="py-3 border bg-slate-400 text-slate-800">
                                No
                            </th>
                            <th
                                class="py-3 border bg-slate-400 text-slate-800 hidden md:table-cell"
                            >
                                Kode
                            </th>
                            <th class="py-3 border bg-slate-400 text-slate-800">
                    ...
                    <tbody>
                        <tr v-if="ekskuls.length < 1">
                            <td
                                colspan="6"
                                class="p-3 bg-slate-200 text-center"
                            >
                        ...
                        >
                            ...
                            <td
                                class="text-center border py-2 hidden md:table-cell"
                            >
                                {{ item.kode }}
                            </td>
                            <td class="text-center border py-2">
                            ...
                            <td
                                class="text-left border py-2 px-2 hidden md:table-cell"
                            >
                                <div class="w-full flex items-center gap-2">
                                    <img
                                        :src="setAvatar(item.guru)"
                                        :alt="item.guru.nama"
                                        class="w-[100px] aspect-square rounded-full shadow"
                                    />
                                    <div>
                            ...
                            <td class="text-center border py-2">
                                {{ item.pesertas.length }}
                            </td>
                            <td class="text-center border py-2">
                                <ul>
                                    <li class="my-1">
                                        <button
                                            class="bg-sky-600 py-1 px-2 rounded text-sm text-white hover:bg-sky-600 w-full"
                                            @click="cetakPresensi(item)"
                                        >
                                            Presensi
                                        </button>
                                    </li>
                                    <li class="my-1">
                                        <button
                                            class="bg-green-600 py-1 px-2 rounded text-sm text-white hover:bg-sky-600 w-full flex items-center gap-1 justify-center"
                                        >
                            ...
                            <td class="text-center border py-2">
                                <button @click="aturPeserta(item)">
                                    <Icon
                                        ...
                                        class="text-sky-400 hover:text-sky-600 text-2xl"
                                    />
                                </button>
    ...
    >
        <div class="dialog-form w-11/12 md:w-[500px] bg-white rounded shadow">
            <div
                class="form-toolbar w-full py-1 px-2 flex items-center justify-between"
            >
                Isi Data Ekskul
                <button @click="closeForm">
                    <Icon
                        icon="mdi:close-circle"
                        class="text-red-600 text-2xl hover:text-red-400"
                    />
                </button>
            </div>
                ...
                <form @submit.prevent="simpan">
                    <label for="Kode" class="w-full flex justify-between my-2">
                        ...
                        <input
                            ...
                            class="py-1 border-none bg-slate-200 w-[60%] rounded"
                            v-model="ekskul.kode"
                            required
                        />
                    </label>
                    <label for="label" class="w-full flex justify-between my-2">
                        ...
                        <input
                            type="text"
                            placeholder="Label Ekskul"
                            class="py-1 border-none bg-slate-200 w-[60%] rounded"
                            v-model="ekskul.label"
                            required
                        />
                    </label>
