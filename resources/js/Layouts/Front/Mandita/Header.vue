...
<template>
    <header class="w-full md:block after:to-[#cce7fef2] after:from-[#f4f8ffb6] overflow-hidden relative bg-[url('/img/muharram-2022.jpg')] bg-no-repeat bg-cover bg-center after:content='' after:absolute after:top-0 after:right-0 after:bottom-0 after:left-0 after:bg-gradient-to-br after:backdrop-blur-sm after:bg-opacity-20">
            ...
            <transition name="fade">
                <article class="-translate-y-16 md:-translate-y-14 bg-white md:w-2/4 md:ml-64 p-3 md:p-10 shadow-xl rounded-xl" v-if="pramuka">
                    <h1 class="flex justify-between">
                        ...
                        <Icon icon="mdi:arrow-left" class="h-6 cursor-pointer" :class="trending == 0 ? 'text-gray-400' : ''" @click="prevTrending" />
                        <Icon icon="mdi:arrow-right" class="h-6 cursor-pointer" :class="trending == (infos.length-1) ? 'text-gray-400' : ''"  @click="nextTrending" />
                    </h1>
                    ...
                    <p class="font-mono text-orange-600 italic flex gap-1 items-center">
                        <Icon icon="mdi:account-circle" class="h-5" />
