...
<div>
    <Head>
        <title>Hasil Pencarian {{searchQuery }}</title>
    </Head>
    <Navbar />
    <section class="min-h-[80vh] relative pb-8">
        ...
        <div class="content w-full sm:w-[80%] md:w-[70%] mx-auto py-8 bg-sky-50 px-4 my-8 shadow-lg">
            <h1 class="font-bold font-serif text-slate-700 text-2xl">Hasil Pencarian dengan Kata Kunci: " <span class="text-teal-600 underline">{{ capitalize(searchQuery) }}</span> "</h1>
            <div class="posts-holder">
                <div class="card flex gap-2 my-4" v-for="(post, p) in posts" :key="post.slug">
                    <img :src="post.featured_image" alt="Cover" class=" h-20 sm:h-28 object-cover aspect-square">
                    <article>
