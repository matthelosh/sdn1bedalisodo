            ...
            <div class="footer-identitas p-3 flex flex-col justify-center items-center md:flex-row gap-3 text-sm" v-if="page.props.sekolah !== null">
                ...
                <div class="footer-identitas--alamat">
                    ...
                    <p  class="leading-4 md:leading-5 tracking-wide text-center md:text-left text-accent flex items-center underline underline-offset-3 gap-1">
                        <Icon icon="email" class="text-md" /> <a :href="`mailto:${page.props.sekolah.email}`">{{ page.props.sekolah.email }}</a>
            ...
            <div class="p-3 text-white">
                ...
                <ul>
                    <li v-for="(post,p) in $page.props.posts" :key="p">
                        <Link class="flex items-center gap-1 text-justify text-accent hover:text-yellow-400" :href="route('Post.read', {kategori:post.category.label.toLowerCase(), slug: post.slug})"> <Icon icon="mdi:chevron-double-right" class="h-3 " /> {{ post.title }}</Link>
                    </li>
